        ...
        <el-table :data="tableList" style="width: 100%">
          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              <template
                v-if="ntime<=Number(dateFormat('YYYYmmdd',new Date(scope.row.examCutoffTime)))&&!(scope.row.isKs)"
              >
                <el-button
                  size='mini'
                  title="参加考试"
                  @click="goExa(scope.row)"
                  type="primary">
                  参加考试
                </el-button>
              </template>

              <span v-else>请先刷新页面</span>
            </template>
          </el-table-column>
        </el-table>
        <pagination
      ...
      swal({
        ...
        if (willDelete) {
          serice({
            ...
            if ('true' == row.frepeat && 'true' == row.ftwice) {
              this.$set(row, 'isKs', true)
            }

            let route = this.$router.resolve({
              name: 'testpaper',
              query: {
                isalone: row.isalone,//答题方式  一道道
                TESTPAPER_ID: data.examinationrecordId,
                FID: row.testpaperId,
                examId: row.examId
              }
            })
