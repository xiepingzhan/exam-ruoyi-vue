      ...
      <el-row>
        ...
        <el-col>
          <el-table :data="tableList" style="width: 100%">
            <el-table-column show-overflow-tooltip label="考试名称" prop="title">
              <template slot-scope="scope">
                <el-link type="primary" :underline="false" @click="openUserList(scope.row)" title="查看试卷">
                  {{ scope.row.examName }}
                </el-link>
              </template>
            </el-table-column>
            <el-table-column show-overflow-tooltip label="试卷名称" prop="title" v-if="false"/>
            ...
            <el-table-column min-width="100" fixed="right" v-if="false" align="center" label="操作">
              <template slot-scope="{row:data}">
                <el-button
                  size='mini'
                  title="删除"
                  @click="goDel(data.testpaperId, data.state)"
